...
public class SIRGroupModel extends AbstractGroupModel<SIRGroup> {
	...
	@Override
	public void initialize(List<Attributes> attributesList, Domain domain, AttributesAgent attributesPedestrian,
			Random random) {
	...
	private void initializeGroupsOfInitialPedestrians() {
		...
		if (c.getElements().size() > 0) {
			// TODO: fill in code to assign pedestrians in the scenario at the beginning
	...
	public void update(final double simTimeInSec) {
		// Update every second independent of the stepSize
		// works only for simTimeStepLength <= 1
		if (simTimeInSec - this.totalTimeInSec < 1) {
			return;
		}
		this.totalTimeInSec++;

		// check the positions of all pedestrians and switch groups to INFECTED (or
		// REMOVED).
		DynamicElementContainer<Pedestrian> c = topography.getPedestrianDynamicElements();
		LinkedCellsGrid<Pedestrian> cellsGrid = topography.getSpatialMap(Pedestrian.class);

		// recover pedestrians
		c.getElements().stream().filter(p -> getGroup(p).getID() == SIRType.ID_INFECTED.ordinal())
				.forEach(p -> recoverWithProbability(p, attributesSIRG.getRecoveryRate()));

		// for the ones who are still infected -> infect others
		c.getElements().stream().filter(p -> getGroup(p).getID() == SIRType.ID_INFECTED.ordinal()).forEach(p -> {
			List<Pedestrian> neighbours = cellsGrid.getObjects(p.getPosition(),
					attributesSIRG.getInfectionMaxDistance());
			for (Pedestrian p_neighbor : neighbours) {
				if (p_neighbor != p && getGroup(p_neighbor).getID() == SIRType.ID_SUSCEPTIBLE.ordinal()) {
					infectWithProbability(p_neighbor, attributesSIRG.getInfectionRate());
				}
			}
	...
	private void infectWithProbability(Pedestrian p, double prob) {
		assignToGroupWithProbability(p, SIRType.ID_INFECTED.ordinal(), prob);
	}

	private void recoverWithProbability(Pedestrian p, double prob) {
		assignToGroupWithProbability(p, SIRType.ID_REMOVED.ordinal(), prob);
	}

	private void assignToGroupWithProbability(Pedestrian p, int groupID, double prob) {
		if (this.random.nextDouble() < prob) {
			elementRemoved(p);
			assignToGroup(p, groupID);
		}
	}
}